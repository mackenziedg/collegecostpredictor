...
```{r}
df2 <- mutate(df1, standardized_score = ((SATVRMID / 800) + (SATMTMID/800) + (ACTCMMID/36))/3  )
...
df2 <- df2 %>% mutate(size_factor = cut(df2$UGDS, levels, labels = labels))
...
#colnames(df2[,51:88])[apply(df2[, 51:88],1,which.max)]

df2$popular_major <- NA
df2$popular_major <- colnames(df2[,51:88])[max.col(df[1:8505,51:88],ties.method="first")]

barplot(prop.table(table(df2$popular_major)))
#the most popular were PCIP51, PCIP52, PCIP24, PCIP11, PCIP50, PCIP47, PCIP39

pop_majors <- df2[popular_major == c('PCIP51', 'PCIP52', 'PCIP24', 'PCIP11', 'PCIP50', 'PCIP47', 'PCIP39'),]
...
#lets figure out how things change across schools with most popluar majors
...
```{r}
#some stats based on your standardized test scores
...
#the higher your SAT score the higher estimated tuition cost
